...
static CSC_STATUS CSCMETHOD CSC_DynamicArrayIContainerInitialize(_When_(return == CSC_STATUS_SUCCESS, _Out_) CONST CSC_PVOID pMemoryBaseAddress, _In_ CONST CSC_SIZE_T elementSize, _In_ CONST CSC_IAllocator* CONST pIAllocator);
...
static CSC_PVOID CSCMETHOD CSC_DynamicArrayIIterableNextElement(_In_ CONST CSC_IIterable* CONST pThis, _In_ CONST CSC_SIZE_T currentIndex, _In_ CONST CSC_PCVOID pCurrentElement);
static CSC_PVOID CSCMETHOD CSC_DynamicArrayIIterableLastElement(_In_ CONST CSC_IIterable* CONST pThis);
static CSC_PVOID CSCMETHOD CSC_DynamicArrayIIterablePreviousElement(_In_ CONST CSC_IIterable* CONST pThis, _In_ CONST CSC_SIZE_T currentIndex, _In_ CONST CSC_PCVOID pCurrentElement);
static CSC_PVOID CSCMETHOD CSC_DynamicArrayIIterableGetElementAt(_In_ CONST CSC_IIterable* CONST pThis, _In_ CONST CSC_SIZE_T index, _In_opt_ CONST CSC_SIZE_T currentIndex, _In_opt_ CONST CSC_PCVOID pCurrentElement);
...
static CSC_STATUS CSCMETHOD CSC_DynamicArrayInitializeWithCopyImpl(_When_(return == CSC_STATUS_SUCCESS, _Out_) CSC_DynamicArray* CONST pThis, _In_ CONST CSC_DynamicArray* CONST pSrc, _In_ CONST CSC_BOOLEAN useInitReserve);
static CSC_STATUS CSCMETHOD CSC_DynamicArrayInitializeWithArrayImpl(_When_(return == CSC_STATUS_SUCCESS, _Out_) CSC_DynamicArray* CONST pThis, _In_ CONST CSC_SIZE_T elementSize, _In_ CONST CSC_SIZE_T numOfElements, _In_ CONST CSC_PCVOID pElements, _In_ CONST CSC_IAllocator* CONST pIAllocator, _In_opt_ CONST CSC_IContainerVirtualTable* CONST pNestedContainerVTable, _In_ CONST CSC_BOOLEAN useInitReserve);
static CSC_STATUS CSCMETHOD CSC_DynamicArrayZeroMemory(_Out_ CSC_DynamicArray* CONST pThis);
...
static CSC_STATUS CSCMETHOD CSC_DynamicArrayInsertRangeImpl(_Inout_ CSC_DynamicArray* CONST pThis, _In_ CONST CSC_SIZE_T insertIndex, _In_ CONST CSC_SIZE_T numOfElements, _In_opt_ CONST CSC_PCVOID pElements, _In_ CONST CSC_BOOLEAN useInitReserve);
...
static CSC_SIZE_T CSCAPI CSC_DynamicArrayCalculateAllocReserve(_In_ CONST CSC_SIZE_T elementSize, _In_ CONST CSC_SIZE_T numOfElements)
{
	...
	if (elementSize > CSC_DYNAMIC_ARRAY_MAXIMUM_SPACE || numOfElements > CSC_DYNAMIC_ARRAY_MAXIMUM_SPACE / elementSize)
	{
		return CSC_DYNAMIC_ARRAY_CALCULATION_ERROR;
	}
...
CSC_STATUS CSCMETHOD CSC_DynamicArrayInitialize(_When_(return == CSC_STATUS_SUCCESS, _Out_) CSC_DynamicArray* CONST pThis, _In_ CONST CSC_SIZE_T elementSize, _In_ CONST CSC_IAllocator* CONST pIAllocator, _In_opt_ CONST CSC_IContainerVirtualTable* CONST pNestedContainerVTable)
{
	...
	status = CSC_DynamicArrayZeroMemory(pThis);
	...
	pThis->baseInterface.pIBaseInterfaceVirtualTable = (CSC_IBaseInterfaceVirtualTable*)&g_dynamicArrayVirtualTable.baseInterfaceVTable;
	pThis->containerInterface.pIContainerVirtualTable = (CSC_IContainerVirtualTable*)&g_dynamicArrayVirtualTable.containerInterfaceVTable;
	pThis->iterableInterface.pIIterableVirtualTable = (CSC_IIterableVirtualTable*)&g_dynamicArrayVirtualTable.iterableInterfaceVTable;
...
CSC_STATUS CSCMETHOD CSC_DynamicArrayInitializeWithSize(_When_(return == CSC_STATUS_SUCCESS, _Out_) CSC_DynamicArray* CONST pThis, _In_ CONST CSC_SIZE_T elementSize, _In_ CONST CSC_SIZE_T numOfElements, _In_opt_ CONST CSC_PCVOID pDefaultValue, _In_ CONST CSC_IAllocator* CONST pIAllocator, _In_opt_ CONST CSC_IContainerVirtualTable* CONST pNestedContainerVTable)
{
	...
	{
		CSC_DynamicArrayZeroMemory(pThis);
		return CSC_STATUS_INVALID_PARAMETER;
	}
	...
	status = CSC_DynamicArrayFillImpl(pThis, numOfElements, pDefaultValue);
...
CSC_STATUS CSCMETHOD CSC_DynamicArrayInitializeWithCopy(_When_(return == CSC_STATUS_SUCCESS, _Out_) CSC_DynamicArray* CONST pThis, _In_ CONST CSC_DynamicArray* CONST pSrc)
{
	return CSC_DynamicArrayInitializeWithCopyImpl(pThis, pSrc, (CSC_BOOLEAN)TRUE);
}
...
static CSC_STATUS CSCMETHOD CSC_DynamicArrayInitializeWithCopyImpl(_When_(return == CSC_STATUS_SUCCESS, _Out_) CSC_DynamicArray* CONST pThis, _In_ CONST CSC_DynamicArray* CONST pSrc, _In_ CONST CSC_BOOLEAN useInitReserve)
{
	...
	{
		status = CSC_DynamicArrayInitializeWithArrayImpl(pThis, pSrc->elementSize, pSrc->elementCount, (CSC_PCVOID)pSrc->pData, pSrc->pIAllocator, pSrc->pNestedContainerVTable, useInitReserve);
	}
	else
	{
...
CSC_STATUS CSCMETHOD CSC_DynamicArrayInitializeWithArray(_When_(return == CSC_STATUS_SUCCESS, _Out_) CSC_DynamicArray* CONST pThis, _In_ CONST CSC_SIZE_T elementSize, _In_ CONST CSC_SIZE_T numOfElements, _In_ CONST CSC_PCVOID pElements, _In_ CONST CSC_IAllocator* CONST pIAllocator, _In_opt_ CONST CSC_IContainerVirtualTable* CONST pNestedContainerVTable)
{
	return CSC_DynamicArrayInitializeWithArrayImpl(pThis, elementSize, numOfElements, pElements, pIAllocator, pNestedContainerVTable, (CSC_BOOLEAN)TRUE);
}
...
static CSC_STATUS CSCMETHOD CSC_DynamicArrayInitializeWithArrayImpl(_When_(return == CSC_STATUS_SUCCESS, _Out_) CSC_DynamicArray* CONST pThis, _In_ CONST CSC_SIZE_T elementSize, _In_ CONST CSC_SIZE_T numOfElements, _In_ CONST CSC_PCVOID pElements, _In_ CONST CSC_IAllocator* CONST pIAllocator, _In_opt_ CONST CSC_IContainerVirtualTable* CONST pNestedContainerVTable, _In_ CONST CSC_BOOLEAN useInitReserve)
{
	...
	status = CSC_DynamicArrayInsertRangeImpl(pThis, (CSC_SIZE_T)0, numOfElements, pElements, useInitReserve);
...
CSC_STATUS CSCMETHOD CSC_DynamicArrayResize(_Inout_ CSC_DynamicArray* CONST pThis, _In_ CONST CSC_SIZE_T numOfElements, _In_opt_ CONST CSC_PCVOID pDefaultValue)
{
	...
	{
		status = CSC_DynamicArrayInitializeWithCopyImpl(&arrayBuffer, pThis, (CSC_BOOLEAN)TRUE);
...
CSC_STATUS CSCMETHOD CSC_DynamicArrayShrinkToFit(_Inout_ CSC_DynamicArray* CONST pThis)
{
	...
	if (pThis->pNestedContainerVTable)
	{
		status = CSC_DynamicArrayInitializeWithCopyImpl(&arrayBuffer, pThis, (CSC_BOOLEAN)FALSE);
...
{
	return CSC_DynamicArrayPopImpl(pThis, pValue, (CSC_BOOLEAN)TRUE, (CSC_BOOLEAN)FALSE);
}
...
{
	return CSC_DynamicArrayPopImpl(pThis, pValue, (CSC_BOOLEAN)FALSE, (CSC_BOOLEAN)FALSE);
}
...
{
	return CSC_DynamicArrayPopImpl(pThis, pValue, (CSC_BOOLEAN)TRUE, (CSC_BOOLEAN)TRUE);
}
...
{
	return CSC_DynamicArrayPopImpl(pThis, pValue, (CSC_BOOLEAN)FALSE, (CSC_BOOLEAN)TRUE);
}
...
static CSC_STATUS CSCMETHOD CSC_DynamicArrayPopImpl(_Inout_ CSC_DynamicArray* CONST pThis, _When_(return == CSC_STATUS_SUCCESS, _Out_opt_) CONST CSC_PVOID pValue, _In_ CONST CSC_BOOLEAN shrink, _In_ CONST CSC_BOOLEAN front)
{
	...
	if (pValue)
	{
		pElement = ((front) ? CSC_DynamicArrayFront(pThis) : CSC_DynamicArrayBack(pThis));
		...
		if (pThis->pNestedContainerVTable)
		{
			if (front || shrink)
			{
				...
				status = CSC_DynamicArrayRemoveRangeImpl(&arrayBuffer, removeIndex, (CSC_SIZE_T)1, shrink);
				...
				pBuffer = pThis->pData;
				reserveSpaceBuffer = pThis->reservedSpace;

				pThis->pData = arrayBuffer.pData;
				pThis->reservedSpace = arrayBuffer.reservedSpace;
				--pThis->elementCount;

				arrayBuffer.pData = pBuffer;
				arrayBuffer.reservedSpace = reserveSpaceBuffer;
			...
			else
			{
				...
				status = CSC_DynamicArrayRemoveRangeImpl(pThis, removeIndex, (CSC_SIZE_T)1, shrink);
	...
	{
		return ((shrink) ? CSC_DynamicArrayRemoveRange(pThis, removeIndex, (CSC_SIZE_T)1) : CSC_DynamicArrayLazyRemoveRange(pThis, removeIndex, (CSC_SIZE_T)1));
	}
}
...
CSC_STATUS CSCMETHOD CSC_DynamicArrayFill(_Inout_ CSC_DynamicArray* CONST pThis, _In_ CONST CSC_SIZE_T numOfElements, _In_opt_ CONST CSC_PCVOID pValue)
{
	...
	status = CSC_DynamicArrayInitialize(&arrayBuffer, pThis->elementSize, pThis->pIAllocator, pThis->pNestedContainerVTable);
	...
	status = CSC_DynamicArrayFillImpl(&arrayBuffer, numOfElements, pValue);
...
static CSC_STATUS CSCMETHOD CSC_DynamicArrayFillImpl(_Inout_ CSC_DynamicArray* CONST pThis, _In_ CONST CSC_SIZE_T numOfElements, _In_opt_ CONST CSC_PCVOID pValue)
{
	...
	{
		CSC_DynamicArrayClear(pThis);
	}

	return CSC_DynamicArrayResizeImpl(pThis, numOfElements, pValue, (CSC_BOOLEAN)FALSE, (CSC_BOOLEAN)TRUE);
}
...
CSC_STATUS CSCMETHOD CSC_DynamicArrayFillRange(_Inout_ CSC_DynamicArray* CONST pThis, _In_ CONST CSC_SIZE_T firstIndex, _In_ CONST CSC_SIZE_T numOfElements, _In_opt_ CONST CSC_PCVOID pValue)
{
	CSC_PVOID pOldData;
	CSC_DynamicArray arrayBuffer;
	CSC_SIZE_T iterator, oldReserve;
	CSC_IContainer* pIteratorIContainer;
	CONST CSC_IContainer* pDefaultValueIContainer;
	CSC_STATUS status = CSC_DynamicArrayIsValid(pThis);
	...
	if (pThis->pNestedContainerVTable)
	{
		...
		{
			pIteratorIContainer = (CSC_IContainer*)CSC_IBaseInterfaceGetInterface((CONST CSC_IBaseInterface* CONST)((CONST CSC_BYTE* CONST)arrayBuffer.pData + arrayBuffer.elementSize * iterator), csc_bit_IContainer);
			status = arrayBuffer.pNestedContainerVTable->pCopy(pIteratorIContainer, pDefaultValueIContainer);
	...
	else
	{
		if (pValue)
		{
			status = CSC_MemoryUtilsSetArrayValue((CSC_PVOID)((CSC_BYTE* CONST)pThis->pData + pThis->elementSize * firstIndex), pValue, pThis->elementSize, numOfElements);
		}
		else
		{
			status = CSC_MemoryUtilsSetZeroMemory((CSC_PVOID)((CSC_BYTE* CONST)pThis->pData + pThis->elementSize * firstIndex), pThis->elementSize * numOfElements);
		}
	}
...
CSC_STATUS CSCMETHOD CSC_DynamicArrayMove(_Inout_ CSC_DynamicArray* CONST pThis, _Inout_ CSC_DynamicArray* CONST pSrc)
{
	...
	if (CSC_DynamicArrayIsValid(pThis) != CSC_STATUS_SUCCESS || CSC_DynamicArrayIsValid(pSrc) != CSC_STATUS_SUCCESS)
	{
		return CSC_STATUS_INVALID_PARAMETER;
	}
	...
	if (pThis->pIAllocator == pSrc->pIAllocator)
	{
		...
		pThis->reservedSpace = pSrc->reservedSpace;
		pThis->pData = pSrc->pData;

		pSrc->elementCount = (CSC_SIZE_T)0;
		pSrc->reservedSpace = (CSC_SIZE_T)0;
		pSrc->pData = NULL;
...
CSC_STATUS CSCMETHOD CSC_DynamicArrayCopyArray(_Inout_ CSC_DynamicArray* CONST pThis, _In_ CONST CSC_SIZE_T elementSize, _In_ CONST CSC_SIZE_T numOfElements, _In_ CONST CSC_PCVOID pElements, _In_opt_ CONST CSC_IContainerVirtualTable* CONST pNestedContainerVTable)
{
	CSC_PVOID pBuffer;
	CSC_SIZE_T oldReserve;
	CSC_DynamicArray arrayBuffer;
	...
	if (!elementSize || !numOfElements || !pElements || elementSize > CSC_DYNAMIC_ARRAY_MAXIMUM_SPACE || numOfElements > CSC_DYNAMIC_ARRAY_MAXIMUM_SPACE / elementSize)
	{
		return CSC_STATUS_INVALID_PARAMETER;
	}

	if (!pNestedContainerVTable && pThis->reservedSpace >= elementSize * numOfElements)
	{
		CSC_DynamicArrayClear(pThis);
		...
		CSC_MemoryUtilsCopyMemory(pThis->pData, pElements, elementSize * numOfElements);
	}
	else
	{
		status = CSC_DynamicArrayInitializeWithArrayImpl(&arrayBuffer, elementSize, numOfElements, pElements, pThis->pIAllocator, pNestedContainerVTable, (CSC_BOOLEAN)FALSE);
		...
		oldReserve = arrayBuffer.reservedSpace;
		pBuffer = arrayBuffer.pData;
		
		arrayBuffer.elementSize = pThis->elementSize;
		arrayBuffer.elementCount = pThis->elementCount;
		arrayBuffer.reservedSpace = pThis->reservedSpace;
		arrayBuffer.pData = pThis->pData;
		arrayBuffer.pNestedContainerVTable = pThis->pNestedContainerVTable;

		pThis->elementSize = elementSize;
		pThis->elementCount = numOfElements;
		pThis->reservedSpace = oldReserve;
		pThis->pData = pBuffer;
		pThis->pNestedContainerVTable = pNestedContainerVTable;
...
CSC_STATUS CSCMETHOD CSC_DynamicArrayInsertRange(_Inout_ CSC_DynamicArray* CONST pThis, _In_ CONST CSC_SIZE_T insertIndex, _In_ CONST CSC_SIZE_T numOfElements, _In_opt_ CONST CSC_PCVOID pElements)
{
	...
	CSC_SIZE_T oldSize, oldReserve;
	CSC_DynamicArray arrayBuffer;
	CSC_STATUS status = CSC_DynamicArrayInitializeWithCopyImpl(&arrayBuffer, pThis, (CSC_BOOLEAN)FALSE);
	...
	status = CSC_DynamicArrayInsertRangeImpl(&arrayBuffer, insertIndex, numOfElements, pElements, (CSC_BOOLEAN)TRUE);
...
static CSC_STATUS CSCMETHOD CSC_DynamicArrayInsertRangeImpl(_Inout_ CSC_DynamicArray* CONST pThis, _In_ CONST CSC_SIZE_T insertIndex, _In_ CONST CSC_SIZE_T numOfElements, _In_opt_ CONST CSC_PCVOID pElements, _In_ CONST CSC_BOOLEAN useInitReserve)
{
	...
	else
	{
		...
		{
			status = CSC_MemoryUtilsCopyMemory(pNewData, pElements, pThis->elementSize * numOfElements);
		}
	}
...
static CSC_STATUS CSCMETHOD CSC_DynamicArrayRemoveImpl(_Inout_ CSC_DynamicArray* CONST pThis, _In_ CONST CSC_SIZE_T removeIndex, _In_ CONST CSC_SIZE_T numOfElements, _In_ CONST CSC_BOOLEAN shrink)
{
	...
	if (pThis->pNestedContainerVTable)
	{
		...
		status = CSC_DynamicArrayRemoveRangeImpl(&arrayBuffer, removeIndex, numOfElements, shrink);
...
CSC_STATUS CSCMETHOD CSC_DynamicArrayReverse(_Inout_ CSC_DynamicArray* CONST pThis)
{
	...
	status = CSC_DynamicArrayResizeImpl(&arrayBuffer, pThis->elementCount, (CSC_PCVOID)NULL, (CSC_BOOLEAN)TRUE, (CSC_BOOLEAN)TRUE);
	...
	for (iterator = (CSC_SIZE_T)0; iterator < pThis->elementCount; ++iterator)
	{
		status = CSC_DynamicArrayInsertRangeImpl(&arrayBuffer, arrayBuffer.elementCount, (CSC_SIZE_T)1, (CSC_PCVOID)((CONST CSC_BYTE* CONST)pThis->pData + pThis->elementSize * (pThis->elementCount - iterator - (CSC_SIZE_T)1)), (CSC_BOOLEAN)FALSE);
...
static CSC_STATUS CSCMETHOD CSC_DynamicArrayIContainerSwapValues(_Inout_ CSC_IContainer* CONST pThis, _In_ CONST CSC_SIZE_T firstIndex, _In_ CONST CSC_SIZE_T secondIndex)
{
	...
	{
		status = CSC_DynamicArrayInitializeWithCopyImpl(&arrayBuffer, pDynamicArray, (CSC_BOOLEAN)FALSE);
		...
		if (status == CSC_STATUS_SUCCESS)
		{
			oldReserve = pDynamicArray->reservedSpace;
			pBuffer = pDynamicArray->pData;

			pDynamicArray->reservedSpace = arrayBuffer.reservedSpace;
			pDynamicArray->pData = arrayBuffer.pData;
...
static CSC_STATUS CSCMETHOD CSC_DynamicArrayIIterableRegisterIterator(_Inout_ CSC_IIterable* CONST pThis, _In_ CONST CSC_IIterator* CONST pIIterator)
{
	...
	{
		pDynamicArray->pIIterator = (CSC_IIterator*)pIIterator;
		return CSC_STATUS_SUCCESS;
	}
}
